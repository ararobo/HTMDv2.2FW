/**
 * @file stm32_fdcan2_driver.cpp
 * @author gn10g (8gn24gn25@gmail.com)
 * @brief STM32のFDCAN2通信用クラス
 * @version 1.0
 * @date 2025-05-10
 *
...
void STM32FDCAN2Driver::init(uint32_t filter_id, uint32_t filter_mask)
{
    // CANのフィルタ設定
...
void STM32FDCAN2Driver::can_callback_process(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    if (hfdcan->Instance == hfdcan2.Instance && RxFifo0ITs == FDCAN_IT_RX_FIFO0_NEW_MESSAGE)
    {
...
void STM32FDCAN2Driver::send(uint16_t id, uint8_t *data, uint8_t len)
{
    if (len > 8)
    {
        Error_Handler();
...
void STM32FDCAN2Driver::receive(uint16_t id, uint8_t *data, uint8_t len)
{
    // 受信処理をオーバーライドしてください
}